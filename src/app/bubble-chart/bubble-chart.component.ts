...
export class BubbleChartComponent implements OnInit {
  ...
  private svg;
  private margin = { top: 40, right: 150, bottom: 60, left: 30 };
  width = 750 - this.margin.left - this.margin.right;
  height = 520 - this.margin.top - this.margin.bottom;

  private createSvg() {
    this.svg = d3
      .select('#My_bubblechart')
      .append('svg')
    ...
    d3.csv('/assets/bubblechartData.csv').then((data) => this.drawChart(data));
  }
  ...
  private drawChart(data: any): void {
    ...
    // Add X axis
    var x = d3.scaleLinear().domain([0, 45000]).range([0, this.width]);
    this.svg
      .append('g')
    ...
    // Add Y axis
    var y = d3.scaleLinear().domain([35, 90]).range([this.height, 0]);
